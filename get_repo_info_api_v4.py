...
access_token = ["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", # Ahura
...
def check_repo_availability(repo_full_name):
    ...
    if body['fork'] == True or body['disabled'] == True or body['private'] == True:
        return False
...
def main():
    global access_token_counter
    data_file_name = "data/keras/repos_keras_code_filtered(fork_star).csv"
    repos = pd.read_csv(data_file_name, low_memory=False)



    # # Backup output
    # csvfile = open('data/pytorch/repos_pytorch_code_filtered(fork_star_activity_contributors)_commit_backup.csv', mode='w')
    ...
    repos.to_csv('data/pytorch/repos_pytorch_code_filtered(fork_star_activity_contributors)_commit.csv', index=False)
    csvfile.close()
